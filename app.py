...
# --- 2. KOMPLETNÍ TEXT PODMÍNEK (30 BODŮ) ---
...
def clean_text(text):
    ...
    # Odstraní vše, co není ASCII (řeší chyby v PDF)
...
# Pomocná logika pro dovyplnění dat
...
if st.button("💾 Save/Update Carrier"):
    if c_name:
        ...
        st.success("Dopravce ulozen v adresari.")
        st.rerun()
...
with c_btn2:
    if st.button("📄 PREPARE PDF", type="primary", use_container_width=True):
        if not ord_num or not c_name:
            st.error("Chybi cislo objednavky nebo jmeno dopravce!")
        else:
            pdf = FPDF()
            pdf.add_page()
            ...
            # --- DOWNLOAD PRO PYTHON 3.13 / STREAMLIT CLOUD ---
            try:
                raw_pdf = pdf.output()
                ...
                st.error(f"Chyba pri generovani: {e}")17. Dispute Resolution The Principal and the Carrier undertake to resolve disputes primarily by mutual agreement. If no agreement is reached, disputes shall be resolved by the court of subject-matter and local jurisdiction in the Czech Republic according to the registered office of the Principal.
18. Assignment of Receivables The Carrier shall not be entitled to assign its receivables from the Principal to third parties without the Principal's prior written consent.
19. Quality Standards and Penalties The Carrier confirms that it has been acquainted with the quality requirements. Penalties for Breach: 
- EUR 100: insufficient securing of cargo (lashing straps, chocks, anti-slip mats). 
- EUR 150: insufficient quality of the vehicle (dirty loading compartment, oil leakage, technical condition). 
...
st.subheader("1. Carrier Selection")
carrier_list = ["-- New Carrier --"] + sorted(df_carriers['nazev'].tolist())
selected_carrier = st.selectbox("Choose from directory", carrier_list)

col1, col2 = st.columns(2)
with col1:
    c_name = st.text_input("Carrier Name", value="" if selected_carrier == "-- New Carrier --" else selected_carrier)
    c_ico = st.text_input("VAT ID / ID", value="" if selected_carrier == "-- New Carrier --" else str(df_carriers[df_carriers['nazev']==selected_carrier]['ico'].values[0]))
    c_tel = st.text_input("Phone", value="" if selected_carrier == "-- New Carrier --" else str(df_carriers[df_carriers['nazev']==selected_carrier]['tel'].values[0]))
with col2:
    c_email = st.text_input("Email", value="" if selected_carrier == "-- New Carrier --" else str(df_carriers[df_carriers['nazev']==selected_carrier]['email'].values[0]))
    c_addr = st.text_area("Full Address", value="" if selected_carrier == "-- New Carrier --" else str(df_carriers[df_carriers['nazev']==selected_carrier]['adresa'].values[0]))
...
st.subheader("2. Transport Logistics")
ord_num = st.text_input("ORDER NUMBER")
col_l, col_u = st.columns(2)
with col_l:
    d_load = st.text_input("Loading Date")
    a_load = st.text_area("Loading Address")
with col_u:
    d_unload = st.text_input("Unloading Date")
    a_unload = st.text_area("Unloading Address")

st.divider()

st.subheader("3. Cargo & Price")
c_qty, c_price = st.columns([1, 1])
qty = c_qty.text_input("Quantity (LF)")
price = c_price.text_input("Price (EUR)")
desc = st.text_area("Description / VIN")

st.divider()

c_btn1, c_btn2 = st.columns(2)
with c_btn1:
    if st.button("🆕 NEW ORDER / CLEAR", use_container_width=True):
        st.rerun()
    ...
    if st.button("📄 PREPARE PDF", type="primary", use_container_width=True):
        if not ord_num or not c_name:
            ...
            try:
                raw_pdf = pdf.output()
                if isinstance(raw_pdf, (bytearray, bytes)):
                    final_data = bytes(raw_pdf)
                else:
                    final_data = raw_pdf.encode('latin-1')
